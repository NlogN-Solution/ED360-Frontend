...
export const MODULE_ROLES = {
  ...
  // Separate from `communication` — Managers get inbox view+assign here that
  // Internal chat doesn't grant them. `integrations` still governs connect/
  ...
  // Gated by `is_platform_admin` (see `canAccessPlatformAdmin`), never by role —
...
/**
 * Platform admin is an orthogonal flag (`User.is_platform_admin`), not a role —
 * an org's own `super_admin` ("Owner") is NOT a platform admin. This gate is
 * independent of `MODULE_ROLES`/`canAccessModule` entirely.
 */
export function canAccessPlatformAdmin(isPlatformAdmin: boolean | undefined): boolean {

...
import { UserPlus, GraduationCap, CheckSquare, Moon, Sun, Monitor, Plus } from "lucide-react";
import {
  CommandDialog,
  CommandEmpty,
  CommandGroup,
...
export function CommandPalette() {
  ...
  const canSearchTasks = canAccessModule(role, "tasks");
  ...
  const { data: taskResults } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<ListResponse<QuickTask>>("/tasks", { params: { search: debouncedQuery, limit: 5 } });
      return data.items;
    },
    enabled: open && debouncedQuery.length > 1 && canSearchTasks,
  });
      ...
      <CommandList>
        ...
        {NAV_GROUPS.map((group) => {
          const items = group.items.filter((item) =>
            item.module === "platform" ? canAccessPlatformAdmin(isPlatformAdmin) : canAccessModule(role, item.module),
          );

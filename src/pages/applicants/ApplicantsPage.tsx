...
import { CommentEntityType, UserRole, UserStatus } from "@/types/enums";
...
export function ApplicantsPage() {
  ...
  const applicantIds = useMemo(() => (data?.items ?? []).map((u) => u.id), [data]);
  const { data: commentCounts } = useCommentCounts(CommentEntityType.APPLICANT, applicantIds);
  ...
  const columns = useMemo<ColumnDef<UserRead, any>[]>(
    () => [
      ...
      { accessorKey: "status", header: "Status", cell: ({ getValue }) => <StatusBadge status={getValue<string>()} /> },
      {
        ...
        cell: ({ row }) => (
          <CommentBadge
            entityType={CommentEntityType.APPLICANT}
            entityId={row.original.id}
            count={commentCounts?.counts[row.original.id] ?? 0}
            detailPath={`/applicants/${row.original.id}#comments`}
          />
        ),
      },
    ],

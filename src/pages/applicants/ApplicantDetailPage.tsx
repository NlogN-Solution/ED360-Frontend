...
import { useEducationHistory, useStudentProfile, useUser } from "@/modules/users/hooks";
...
import { CommentEntityType } from "@/types/enums";
...
export function ApplicantDetailPage() {
  ...
  const { data: education } = useEducationHistory(userId);
  const canSeeSensitive = isManagerRole(role);

  const latestGrade = useMemo(() => {
    if (!education || education.length === 0) return null;
    return [...education].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0].grade;
  }, [education]);
    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="space-y-4 lg:col-span-2">
          <section className="rounded-xl border border-border bg-card p-4">
            ...
            {profileLoading ? (
              ...
              <div className="grid grid-cols-2 gap-4 text-sm sm:grid-cols-3">
                ...
                <Field label="GPA" value={latestGrade ?? (profile.gpa ? String(profile.gpa) : undefined)} />
              </div>
            )}
            {profile?.notes && (
          ...
          <section id="comments" className="scroll-mt-20 rounded-xl border border-border bg-card p-4">
            <h2 className="mb-3 text-[13px] font-semibold text-foreground">Comments</h2>
            <CommentsPanel entityType={CommentEntityType.APPLICANT} entityId={user.id} />
          </section>
              ...
              <div className="divide-y divide-border">
                {applications.items.map((app) => (
                  <div
                    ...
                    className="flex w-full cursor-pointer items-center justify-between py-2.5 text-left transition-colors hover:bg-muted/30"
                  >
                    <div className="flex items-center gap-2">
                      <FileText className="h-3.5 w-3.5 text-muted-foreground" />
                      <ProgramNameCell programId={app.program_id} linkToDetail={false} />
                    </div>
                    <div className="flex items-center gap-2">

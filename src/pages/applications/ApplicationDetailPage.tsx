...
import { ArrowLeft, Building2, Clock, FileText, Globe2, GraduationCap, Pencil, Route, Sparkles, Trash2, Wallet } from "lucide-react";
...
import { useCountry, useProgram, useUniversity } from "@/modules/academic/hooks";
...
import { ApplicationEditDialog } from "@/modules/applications/ApplicationEditDialog";
...
export function ApplicationDetailPage() {
  ...
  return (
    <div>
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <InfoRow
                ...
                value={
                  university ? (
                    <LinkValue onClick={() => navigate(`/academic/universities/${university.id}`)}>{university.name}</LinkValue>
                  ) : (
                    "—"
                  )
                }
              ...
              <InfoRow
                ...
                value={
                  program ? (
                    <LinkValue onClick={() => navigate(`/academic/programs/${program.id}`)}>{program.name}</LinkValue>
                  ) : (
                    "—"
                  )
                }
              ...
              <InfoRow
                ...
                value={
                  country ? (
                    <LinkValue onClick={() => navigate(`/academic/countries/${country.id}`)}>{country.name}</LinkValue>
                  ) : (
                    "—"
                  )
                }
      ...
      {!isStudent && (
        <div id="comments" className="mt-4 scroll-mt-20 rounded-xl border border-border bg-card p-4">
          <h2 className="mb-3 text-[13px] font-semibold text-foreground">Comments</h2>
          <CommentsPanel entityType={CommentEntityType.APPLICATION} entityId={application.id} />
        </div>
      )}
      ...
      {canManage && <ApplicationEditDialog application={application} open={editOpen} onOpenChange={setEditOpen} />}
    </div>
  );
}
...
function LinkValue({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
    <button className="text-left hover:text-primary hover:underline" onClick={onClick}>
      {children}
    </button>
  );
}

...
function CountriesTab() {
  ...
  const { data, isLoading } = useCountries({ search: debouncedSearch || undefined, limit: 100 });
  ...
  return (
    <div className="space-y-3">
      ...
      <ListToolbar searchValue={search} onSearchChange={setSearch} searchPlaceholder="Search countries…" />
        ...
        <div className="divide-y divide-border rounded-xl border border-border bg-card">
          {data.items.map((c) => (
            <div
              ...
              role="button"
              tabIndex={0}
              onClick={() => navigate(`/academic/countries/${c.id}`)}
              onKeyDown={(e) => e.key === "Enter" && navigate(`/academic/countries/${c.id}`)}
              className="flex w-full cursor-pointer items-center justify-between px-4 py-2.5 text-left transition-colors hover:bg-muted/30"
            >
...
function UniversitiesTab() {
  ...
  const [search, setSearch] = useState("");
  const debouncedSearch = useDebounce(search);
  const { data, isLoading } = useUniversities({ search: debouncedSearch || undefined, limit: 100 });
  ...
  return (
    <div className="space-y-3">
      ...
      <ListToolbar searchValue={search} onSearchChange={setSearch} searchPlaceholder="Search universities…" />
        ...
        <div className="divide-y divide-border rounded-xl border border-border bg-card">
          {data.items.map((u) => (
            <div
              ...
              onKeyDown={(e) => e.key === "Enter" && navigate(`/academic/universities/${u.id}`)}
              className="flex w-full cursor-pointer items-center justify-between px-4 py-2.5 text-left transition-colors hover:bg-muted/30"
            >
              <div className="flex items-center gap-2.5">
...
function ProgramsTab() {
  ...
  const [search, setSearch] = useState("");
  const debouncedSearch = useDebounce(search);
  const { data, isLoading } = usePrograms({ search: debouncedSearch || undefined, limit: 100 });
  ...
  return (
    <div className="space-y-3">
      ...
      <ListToolbar searchValue={search} onSearchChange={setSearch} searchPlaceholder="Search courses…" />
        ...
        <div className="divide-y divide-border rounded-xl border border-border bg-card">
          {data.items.map((p) => (
            <div
              ...
              onKeyDown={(e) => e.key === "Enter" && navigate(`/academic/programs/${p.id}`)}
              className="flex w-full cursor-pointer items-center justify-between px-4 py-2.5 text-left transition-colors hover:bg-muted/30"
            >
              <div className="flex items-center gap-2.5">

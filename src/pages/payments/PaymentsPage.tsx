...
import { Bell, Clock, Loader2, Plus, Receipt, Wallet } from "lucide-react";
...
import { usePayments, useRemindPayment } from "@/modules/payments/hooks";
...
function RemindButton({ paymentId }: { paymentId: string }) {
  const remind = useRemindPayment();
  return (
    <Button variant="outline" size="sm" disabled={remind.isPending} onClick={() => remind.mutate(paymentId)}>
      {remind.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Bell className="h-3.5 w-3.5" />}
      Remind
    </Button>
  );
}
        ...
        : [
          {
            ...
            header: "",
            cell: ({ row }) =>
              row.original.status === PaymentStatus.PENDING || row.original.status === PaymentStatus.PROCESSING ? (
                <RemindButton paymentId={row.original.id} />
              ) : null,
          } as ColumnDef<PaymentRead, any>,
        ]),
    ],

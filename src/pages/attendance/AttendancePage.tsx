  ...
  const columns = useMemo<ColumnDef<AttendanceRecord, any>[]>(
    () => [
      ...
      { accessorKey: "status", header: "Status", cell: ({ getValue }) => <StatusBadge status={getValue<string>()} /> },
      {
        ...
        header: "Notes",
        cell: ({ row }) => {
          const { check_in_notes, check_out_notes } = row.original;
          if (!check_in_notes && !check_out_notes) return <span className="text-muted-foreground">—</span>;
          return (
            <span
              className="line-clamp-1 text-xs text-muted-foreground"
              title={[check_in_notes && `Plan: ${check_in_notes}`, check_out_notes && `Summary: ${check_out_notes}`]
                .filter(Boolean)
                .join("\n")}
            >
              {check_in_notes && `Plan: ${check_in_notes}`}
              {check_in_notes && check_out_notes && " · "}
              {check_out_notes && `Summary: ${check_out_notes}`}
            </span>
          );
        },
      },

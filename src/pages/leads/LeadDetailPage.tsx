...
import { CommentEntityType, LeadStatus, UserRole } from "@/types/enums";
...
export function LeadDetailPage() {
  ...
  const { data: commentCounts } = useCommentCounts(CommentEntityType.LEAD, leadId ? [leadId] : []);
  const commentCount = leadId ? (commentCounts?.counts[leadId] ?? 0) : 0;
    ...
    <div>
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
        ...
        {canManage && (
          <div className="flex items-center gap-2">
            ...
            {(isManagerRole(role) || role === UserRole.COUNSELLOR) && (
              <Button
                variant="outline"
                size="sm"
                onClick={() => {
              ...
              >
                <UserCog className="h-3.5 w-3.5" /> {lead.assigned_to ? "Reassign" : "Assign"}
              </Button>
            )}
      ...
      <Tabs value={activeTab} onValueChange={(v) => setSearchParams((prev) => ({ ...Object.fromEntries(prev), tab: v }))}>
        ...
        <TabsContent value="comments" className="mt-4">
          <div className="rounded-xl border border-border bg-card p-4">
            <CommentsPanel entityType={CommentEntityType.LEAD} entityId={lead.id} />
          </div>
        </TabsContent>
      </Tabs>
      ...
      <Dialog open={assignOpen} onOpenChange={setAssignOpen}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>{lead.assigned_to ? "Reassign lead" : "Assign lead"}</DialogTitle>
          </DialogHeader>

...
export function useCountry(id: string | undefined) {
  return useQuery({
    queryKey: ["academic", "country-detail", id],
    queryFn: () => academicService.countries.get(id as string),
    enabled: Boolean(id),
    staleTime: 5 * 60 * 1000,
  });
}
...
export function useUpdateCountry(id: string) {
  const invalidate = useInvalidateAcademic();
  return useMutation({
    mutationFn: (payload: Partial<CountryPayload>) => academicService.countries.update(id, payload),
    onSuccess: () => {
      invalidate();
...
export function useUpdateUniversity(id: string) {
  const invalidate = useInvalidateAcademic();
  return useMutation({
    mutationFn: (payload: Partial<UniversityPayload>) => academicService.universities.update(id, payload),
    onSuccess: () => {
      invalidate();
...
export function useUpdateProgram(id: string) {
  const invalidate = useInvalidateAcademic();
  return useMutation({
    mutationFn: (payload: Partial<ProgramPayload>) => academicService.programs.update(id, payload),
    onSuccess: () => {
      invalidate();

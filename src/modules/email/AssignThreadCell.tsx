...
import { useAssignEmailThread } from "./hooks";
import { UserRole } from "@/types/enums";
import { cn } from "@/lib/utils";

// Same searchable-staff-popover pattern as modules/whatsapp/AssignConversationCell.tsx.
export function AssignThreadCell({ threadId, assignedTo }: { threadId: string; assignedTo: string | null }) {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState("");
  const debouncedSearch = useDebounce(search, 250);
  const assign = useAssignEmailThread();
            ...
            <CommandGroup>
              {assignedTo && (
                <CommandItem
                  ...
                  onSelect={() => {
                    setOpen(false);
                    assign.mutate({ threadId, assignedTo: null });
                  }}
                >
              ...
              {data?.items.map((user) => (
                <CommandItem
                  ...
                  onSelect={() => {
                    setOpen(false);
                    if (user.id !== assignedTo) assign.mutate({ threadId, assignedTo: user.id });
                  }}
                >

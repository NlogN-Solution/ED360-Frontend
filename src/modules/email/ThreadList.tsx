...
import { useEmailThreads } from "./hooks";
...
export function ThreadList({ activeId, onSelect }: ThreadListProps) {
  ...
  const { data, isLoading } = useEmailThreads(params);
  const items = (data?.items ?? []).filter((t) => (filter === "unread" ? t.unread_count > 0 : true));
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="Try another name, address, or filter."
            ...
            >
              <div className="min-w-0 flex-1">
                <p className="truncate text-[13px] font-medium text-foreground">
                  {thread.contact.display_name || thread.contact.email_address}
                </p>

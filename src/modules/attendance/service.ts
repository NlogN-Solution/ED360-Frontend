...
export const attendanceService = {
  ...
  async checkIn(notes?: string): Promise<AttendanceRecord> {
    const { data } = await apiClient.post<AttendanceRecord>("/attendance/check-in", { notes: notes || undefined });
    return data;
  },

  async checkOut(notes?: string): Promise<AttendanceRecord> {
    const { data } = await apiClient.post<AttendanceRecord>("/attendance/check-out", { notes: notes || undefined });
    return data;
  },

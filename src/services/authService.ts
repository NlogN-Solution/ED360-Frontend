...
export const authService = {
  ...
  async impersonate(userId: string): Promise<TokenResponse> {
    const { data } = await apiClient.post<TokenResponse>(`/auth/impersonate/${userId}`);
    return data;
  },

  async exitImpersonation(impersonatorId: string): Promise<void> {
    await apiClient.post("/auth/impersonate/exit", { impersonator_id: impersonatorId });
  },
